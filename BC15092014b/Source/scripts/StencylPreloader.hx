...
class StencylPreloader extends Sprite
{
	...
	public function new()
	{
		...
		var r = new nme.geom.Rectangle(0, 0, 0, barHeight);
		bar.scrollRect = r;
	...
	public function showLockScreen(realURL:String)
	{
		...
		var tmp = new nme.display.Bitmap(new nme.display.BitmapData(Std.int(getWidth()), Std.int(getHeight()), false, 0x565656));
		addChild(tmp);
		...
		txt.addEventListener(nme.events.MouseEvent.CLICK, goToLockURL);
		tmp.addEventListener(nme.events.MouseEvent.CLICK, goToLockURL);
	}
	...
	{
		nme.Lib.getURL(new nme.net.URLRequest(getLockURL()), "_parent");
	}
	
	public function onMouseDown(e:MouseEvent):Void
	{
		removeEventListener(MouseEvent.MOUSE_DOWN, onMouseDown);
		nme.Lib.getURL(new nme.net.URLRequest(getURL()), "_blank");
	}
	...
	{
		return Std.int(scripts.MyAssets.stageWidth * Engine.SCALE);
	}
	
	#if(flash || js) override #end public function getHeight():Float
	{
		return Std.int(scripts.MyAssets.stageHeight * Engine.SCALE);
	}
	...
	#if(flash || js) override #end public function onUpdate(bytesLoaded:Int, bytesTotal:Int)
	{
		...
		if(bar != null)
		{
			...
			var r = new nme.geom.Rectangle(0, 0, barWidth * percentLoaded, barHeight);
			bar.scrollRect = r;
		}
	}
